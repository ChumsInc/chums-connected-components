...
export const selectCurrentPage = (key: string) => (state: RootStateWithPages): number => state.pageSets[key]?.page ?? defaultPageSet.page;
export const selectRowsPerPage = (key: string) => (state: RootStateWithPages): number => state.pageSets[key]?.rowsPerPage ?? defaultPageSet.rowsPerPage;
export const selectPageSet = (key:string) => (state:RootStateWithPages):PageSet => ({...defaultPageSet, ...state.pageSets[key]});
